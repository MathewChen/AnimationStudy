...
- (UIFont *)placeholdFont {
    return objc_getAssociatedObject(self, &PLACEHOLDFONT) == nil ? [UIFont systemFontOfSize:14.] : objc_getAssociatedObject(self, &PLACEHOLDFONT);
}
...
- (void)setWordcountLable:(NSNumber *)limitLength {
     ...
     *  字数限制
     */
    self.wordCountLabel = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetMinX(self.frame), CGRectGetMaxY(self.frame), CGRectGetWidth(self.frame), 20)];
...
- (void)layoutSubviews {
    ...
    if (self.limitLength && self.wordCountLabel) {
         ...
         */
        self.wordCountLabel.frame = CGRectMake(CGRectGetMinX(self.frame), CGRectGetMaxY(self.frame), CGRectGetWidth(self.frame), 20);
    }
