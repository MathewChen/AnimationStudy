...
#pragma mark - Global Color
    ...
    QMUICMI.navBarBackgroundImage = [[UIImage qmui_imageWithColor:self.themeTintColor] resizableImageWithCapInsets:UIEdgeInsetsMake(0, 1, 0, 1) resizingMode:UIImageResizingModeStretch];
//    QMUICMI.navBarBackgroundImage = [UIImageMake(@"navigationbar_background") resizableImageWithCapInsets:UIEdgeInsetsMake(0, 1, 0, 1) resizingMode:UIImageResizingModeStretch];   // NavBarBackgroundImage : UINavigationBar 的背景图，注意 navigationBar 的高度会受多个因素（是否全面屏、是否使用了 navigationItem.prompt、是否将 UISearchBar 作为 titleView）的影响，要检查各种情况是否都显示正常。
    ...
    QMUICMI.searchBarPlaceholderColor = [UIColor qmui_colorWithThemeProvider:^UIColor * _Nonnull(__kindof QMUIThemeManager * _Nonnull manager, NSString * _Nullable identifier, NSObject<MXThemeProtocol> *theme) {
    ...
    QMUICMI.tableViewCellDisclosureIndicatorImage = [UIImage qmui_imageWithThemeProvider:^UIImage * _Nonnull(__kindof QMUIThemeManager * _Nonnull manager, NSString * _Nullable identifier, NSObject<MXThemeProtocol> * _Nullable theme) {
    return [identifier isEqualToString:MXThemeIdentifierDark] ? disclosureIndicatorImageDark : disclosureIndicatorImage;
    ...
    QMUICMI.tableViewGroupedBackgroundColor = [UIColor qmui_colorWithThemeProvider:^UIColor * _Nonnull(__kindof QMUIThemeManager * _Nonnull manager, NSString * _Nullable identifier, NSObject<MXThemeProtocol> * _Nullable theme) {
        if ([identifier isEqualToString:MXThemeIdentifierDark]) {
...
- (BOOL)shouldApplyTemplateAutomatically {
    ...
    NSString *selectedThemeIdentifier = [[NSUserDefaults standardUserDefaults] stringForKey:MXSelectedThemeIdentifier];
